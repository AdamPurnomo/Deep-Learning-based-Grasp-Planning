...
void computeTetrahedronInertia(btScalar mass, btVector3* points, btMatrix3x3& inertia) {
    ...
    for (uint32_t i = 0; i < 3; ++i ) {
        ...
        inertia[j][k] = inertia[k][j] = - mass * btScalar(0.05) *
            ( btScalar(2.0) * ( p0[j] * p0[k] +  p1[j] * p1[k] +  p2[j] * p2[k] +  p3[j] * p3[k] )
            + p0[j] * (p1[k] + p2[k] + p3[k])
            + p1[j] * (p0[k] + p2[k] + p3[k])
            + p2[j] * (p0[k] + p1[k] + p3[k])
            + p3[j] * (p0[k] + p1[k] + p2[k]) );
...
void computeTetrahedronInertiaByBruteForce(btVector3* points, btMatrix3x3& inertia) {
    ...
    // compute normals
    btVector3 center = btScalar(0.25) * (points[0] + points[1] + points[2] + points[3]);
    btVector3 normals[4];
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        if (normals[i].dot(p0 - center) < btScalar(0.0)) {
            normals[i] *= btScalar(-1.0);
        }
        pointsOnPlane[i] = p0;
    }
    ...
    btScalar resolutionOfIntegration = btScalar(400.0);
    ...
    while(x < XX) {
        ...
        while (y < YY) {
            ...
            while (z < ZZ) {
                ...
                for (int i = 0; i < 4; ++i) {
                    if ((p - pointsOnPlane[i]).dot(normals[i]) > btScalar(0.0)) {
                        pointInside = false;
                        break;
                    }
                }
...
btScalar computeTetrahedronVolume(btVector3* points) {
    ...
    return ((points[2] - points[1]).cross(points[3] - points[2])).dot(points[3] - points[0]) / btScalar(6.0);
}
...
void applyParallelAxisTheorem(btMatrix3x3& inertia, const btVector3& shift, btScalar mass) {
	...
	if (distanceSquared > btScalar(0.0)) {
		for (uint32_t i = 0; i < 3; ++i) {
...
void MeshMassProperties::computeMassProperties(const VectorOfPoints& points, const VectorOfIndices& triangleIndices) {
    ...
    const btVector3 p0(0.0, 0.0, 0.0);
    ...
    for (uint32_t i = 0; i < numTriangles; ++i) {
        ...
        center = btScalar(0.25) * (tetraPoints[1] + tetraPoints[2] + tetraPoints[3]);
