...
pointmap = np.loadtxt(r"../../Test Data/Point Map Data/hv18/pointmap0.txt")
...
data_dir = r"../../Test Data/Mask/hv18"
...
for im_path in file_path:
    ...
    #vector matrix
    list_form = label_path.split(os.path.sep)
    label_path = list_form
    label_path[-2] = 'vector matrix'
    label_path = os.path.sep.join(label_path)
    label = np.load(label_path)
    vector_matrix.append(label)
...
model.load_weights(r"../../Output/Weights/MaskPredictor_hv18_v2")
s_prediction = []
r_prediction = []
@tf.function
