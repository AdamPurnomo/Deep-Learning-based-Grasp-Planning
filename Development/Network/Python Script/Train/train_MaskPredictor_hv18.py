...
data_path = r'..\..\Training Data\Mask\hv18'
...
train_cost = m.BinaryFocalLoss(scale= 200)
val_cost = m.BinaryFocalLoss(scale= 200)
...
lossfunc = lf.BinaryFocalLoss(scale= 200)

#instantiating model
d_model = Discriminator()
d_model.build((1,220,100,3))
d_model.load_weights(r'..\..\Output\Weights\Discriminator_hv18')
model = MaskPredictor()
...
for epoch in range(Epochs):
    ...
    #tm.append(train_mse.result())
    #vm.append(train_mse.result())
    
    #saving for check point
    base = r'..\..\Output\tmp\weight check point'
    dir = base + r'\Epoch_' + str(18 + epoch)
    model.save_weights(dir)

model.save_weights(r'..\..\Output\Weights\MaskPredictor_hv18.h5')
model.save_weights(r'..\..\Output\Weights\MaskPredictor_hv18')
N = np.arange(0, Epochs)
History = np.array([N, tl, tp, tm, vl, vp, vm])
np.save(r'..\..\Output\History\MaskPredictor_hv18.npy', History)
