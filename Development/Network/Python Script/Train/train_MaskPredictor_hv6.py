...
data_path = r'..\..\Training Data\Mask\hv6'
...
d_model.load_weights(r'..\..\Training Output\Weights\Discriminator_hv6')
model = MaskPredictor()
...
for epoch in range(Epochs):
    ...
    for i in range(tr_step):
        ...
        full_im = np.expand_dims(train_x[i], axis = 0)
        label = np.expand_dims(train_y[i], axis = (-1,0))
...
model.save_weights(r'..\..\Training Output\Weights\MaskPredictor_hv6.h5')
model.save_weights(r'..\..\Training Output\Weights\MaskPredictor_hv6')
N = np.arange(0, Epochs)
History = np.array([N, tl, tp, tm, vl, vp, vm])
np.save(r'..\..\Training Output\History\MaskPredictor_hv6.npy', History)
